...
const DetallesPedidosPorComada = (props) => {
    ...
    const getDetalles = async () => {
        var result = await fetch(`${global.config.appConfig.url.dev}POC_DetallePedido?pedido=${pedido}&producto=${producto}`, {
            method: 'GET',
            headers: global.config.appConfig.headers.dev,
            json: true
        }).then(response => {
    ...
    const upEstadoDetallePedido = async (detalle) => {
        await fetch(`${global.config.appConfig.url.dev}POC_DetallePedido/${detalle.DetallePedidoId}?estado=${detalle.EstadoId}`, {
            ...
            fetch(`${global.config.appConfig.url.dev}POC_DetallePedido?pedido=${pedido}&producto=${producto}`)
                ...
                <Grid container spacing={0}>
                    <Grid item xs={10} md={10} lg={10}>
                        <Breadcrumbs aria-label="breadcrumb">
                            ...
                            <Link color="inherit" href={`/pedidosComandas/${comanda}`} >Pedidos</Link>
...
export default DetallesPedidosPorComada;